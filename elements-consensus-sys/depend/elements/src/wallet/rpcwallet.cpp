...
namespace {
static rustsecp256k1_v0_4_1_context *rustsecp256k1_v0_4_1_ctx;

class CSecp256k1Init {
public:
    CSecp256k1Init() {
        rustsecp256k1_v0_4_1_ctx = rustsecp256k1_v0_4_1_context_create(SECP256K1_CONTEXT_VERIFY | SECP256K1_CONTEXT_SIGN);
    }
    ~CSecp256k1Init() {
        rustsecp256k1_v0_4_1_context_destroy(rustsecp256k1_v0_4_1_ctx);
    }
};
...
bool DerivePubTweak(const std::vector<uint32_t>& vPath, const CPubKey& keyMaster, const ChainCode &ccMaster, std::vector<unsigned char>& tweakSum)
{
    ...
    for (unsigned int i = 0; i < vPath.size(); i++) {
        ...
        if (i == 0) {
            ...
            bool ret = rustsecp256k1_v0_4_1_ec_privkey_tweak_add(rustsecp256k1_v0_4_1_ctx, tweakSum.data(), tweak.data());
            if (!ret) {
                return false;
            }
        }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    rustsecp256k1_v0_4_1_pubkey masterpub_secp;
    int ret = rustsecp256k1_v0_4_1_ec_pubkey_parse(rustsecp256k1_v0_4_1_ctx, &masterpub_secp, masterpub.begin(), masterpub.size());
    if (ret != 1) {
    ...
    // Negate the pubkey
    ret = rustsecp256k1_v0_4_1_ec_pubkey_negate(rustsecp256k1_v0_4_1_ctx, &masterpub_secp);

    std::vector<unsigned char> negatedpubkeybytes;
    negatedpubkeybytes.resize(33);
    size_t len = 33;
    ret = rustsecp256k1_v0_4_1_ec_pubkey_serialize(rustsecp256k1_v0_4_1_ctx, &negatedpubkeybytes[0], &len, &masterpub_secp, SECP256K1_EC_COMPRESSED);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    rustsecp256k1_v0_4_1_pubkey onlinepubkey_secp;
    if (rustsecp256k1_v0_4_1_ec_pubkey_parse(rustsecp256k1_v0_4_1_ctx, &onlinepubkey_secp, onlinepubkey.begin(), onlinepubkey.size()) != 1) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Pubkey is invalid");
    }

    // Get index of given online key
    int whitelistindex=-1;
    std::vector<rustsecp256k1_v0_4_1_pubkey> pak_online = paklist.OnlineKeys();
    for (unsigned int i=0; i<pak_online.size(); i++) {
        if (memcmp((void *)&pak_online[i], (void *)&onlinepubkey_secp, sizeof(rustsecp256k1_v0_4_1_pubkey)) == 0) {
            whitelistindex = i;
            break;
        }
    }
    ...
    rustsecp256k1_v0_4_1_pubkey btcpub_secp;
    memcpy(&btcpub_secp, &masterpub_secp, sizeof(rustsecp256k1_v0_4_1_pubkey));

    // Negate master pubkey
    ret = rustsecp256k1_v0_4_1_ec_pubkey_negate(rustsecp256k1_v0_4_1_ctx, &masterpub_secp);

    // Make sure negated master pubkey is in PAK list at same index as online_pubkey
    if (memcmp((void *)&paklist.OfflineKeys()[whitelistindex], (void *)&masterpub_secp, sizeof(rustsecp256k1_v0_4_1_pubkey)) != 0) {
    ...
    ret = rustsecp256k1_v0_4_1_ec_pubkey_tweak_add(rustsecp256k1_v0_4_1_ctx, &btcpub_secp, tweakSum.data());
    CHECK_NONFATAL(ret);

    std::vector<unsigned char> btcpubkeybytes;
    btcpubkeybytes.resize(33);
    size_t btclen = 33;
    ret = rustsecp256k1_v0_4_1_ec_pubkey_serialize(rustsecp256k1_v0_4_1_ctx, &btcpubkeybytes[0], &btclen, &btcpub_secp, SECP256K1_EC_COMPRESSED);
    CHECK_NONFATAL(ret == 1);
    CHECK_NONFATAL(btclen == 33);
    CHECK_NONFATAL(btcpubkeybytes.size() == 33);

    //Create, verify whitelist proof
    rustsecp256k1_v0_4_1_whitelist_signature sig;
    if(rustsecp256k1_v0_4_1_whitelist_sign(rustsecp256k1_v0_4_1_ctx, &sig, &paklist.OnlineKeys()[0], &paklist.OfflineKeys()[0], paklist.size(), &btcpub_secp, masterOnlineKey.begin(), &tweakSum[0], whitelistindex, NULL, NULL) != 1) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Pegout authorization proof signing failed");
    }

    if (rustsecp256k1_v0_4_1_whitelist_verify(rustsecp256k1_v0_4_1_ctx, &sig, &paklist.OnlineKeys()[0], &paklist.OfflineKeys()[0], paklist.size(), &btcpub_secp) != 1) {
    ...
    rustsecp256k1_v0_4_1_whitelist_signature_serialize(rustsecp256k1_v0_4_1_ctx, output, &outlen, &sig);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    rustsecp256k1_v0_4_1_pubkey onlinepubkey_secp;
    if (!rustsecp256k1_v0_4_1_ec_pubkey_parse(rustsecp256k1_v0_4_1_ctx, &onlinepubkey_secp, &onlinepubkeybytes[0], onlinepubkeybytes.size()))
        throw JSONRPCError(RPC_WALLET_ERROR, "Invalid online pubkey");
    ...
    std::vector<rustsecp256k1_v0_4_1_pubkey> pak_online = paklist.OnlineKeys();
    for (unsigned int i=0; i<pak_online.size(); i++) {
        if (!memcmp((void *)&pak_online[i], (void *)&onlinepubkey_secp, sizeof(rustsecp256k1_v0_4_1_pubkey)))
            whitelistindex = i;
    }
    if (whitelistindex == -1)
        throw JSONRPCError(RPC_WALLET_ERROR, "Given online key is not in Pegout Authorization Key List");
    ...
    rustsecp256k1_v0_4_1_pubkey btcpubkey;
    if (rustsecp256k1_v0_4_1_ec_pubkey_parse(rustsecp256k1_v0_4_1_ctx, &btcpubkey, &btcpubkeybytes[0], btcpubkeybytes.size()) != 1)
        throw JSONRPCError(RPC_WALLET_ERROR, "btcpubkey is invalid pubkey");

    //Create, verify whitelist proof
    rustsecp256k1_v0_4_1_whitelist_signature sig;
    if(rustsecp256k1_v0_4_1_whitelist_sign(rustsecp256k1_v0_4_1_ctx, &sig, &paklist.OnlineKeys()[0], &paklist.OfflineKeys()[0], paklist.size(), &btcpubkey, masterOnlineKey.begin(), &sumprivkeybytes[0], whitelistindex, NULL, NULL) != 1)
        throw JSONRPCError(RPC_WALLET_ERROR, "Pegout authorization proof signing failed");

    if (rustsecp256k1_v0_4_1_whitelist_verify(rustsecp256k1_v0_4_1_ctx, &sig, &paklist.OnlineKeys()[0], &paklist.OfflineKeys()[0], paklist.size(), &btcpubkey) != 1)
        throw JSONRPCError(RPC_WALLET_ERROR, "Pegout authorization proof was created and signed but is invalid");
    ...
    CHECK_NONFATAL(1 + 32 * (1 + 256) >= expectedOutputSize);
    unsigned char output[1 + 32 * (1 + 256)];
    rustsecp256k1_v0_4_1_whitelist_signature_serialize(rustsecp256k1_v0_4_1_ctx, output, &expectedOutputSize, &sig);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!rustsecp256k1_v0_4_1_ec_privkey_tweak_add(rustsecp256k1_v0_4_1_ctx, &pegoutkeybytes[0], &pegoutsubkeybytes[0]))
