...
void bench_scalar_sqr(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_4_1_scalar_sqr(&data->scalar[0], &data->scalar[0]);
    }
}
...
void bench_group_jacobi_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        j += rustsecp256k1_v0_4_1_gej_has_quad_y_var(&data->gej[0]);
        /* Vary the Y and Z coordinates of the input (the X coordinate doesn't matter to
           rustsecp256k1_v0_4_1_gej_has_quad_y_var). Note that the resulting coordinates will
           ...
           curve). */
        rustsecp256k1_v0_4_1_fe_add(&data->gej[0].y, &data->fe[1]);
        rustsecp256k1_v0_4_1_fe_add(&data->gej[0].z, &data->fe[2]);
...
void bench_num_jacobi(void* arg, int iters) {
    int i, j = 0;
    bench_inv *data = (bench_inv*)arg;
    rustsecp256k1_v0_4_1_num nx, na, norder;

    rustsecp256k1_v0_4_1_scalar_get_num(&nx, &data->scalar[0]);
    rustsecp256k1_v0_4_1_scalar_order_get_num(&norder);
    rustsecp256k1_v0_4_1_scalar_get_num(&na, &data->scalar[1]);

    for (i = 0; i < iters; i++) {
        j += rustsecp256k1_v0_4_1_num_jacobi(&nx, &norder);
        rustsecp256k1_v0_4_1_num_add(&nx, &nx, &na);
    }
    CHECK(j <= iters);
}
#endif
