...
rustsecp256k1_v0_4_1_context* rustsecp256k1_v0_4_1_context_create(unsigned int flags) {
    size_t const prealloc_size = rustsecp256k1_v0_4_1_context_preallocated_size(flags);
    rustsecp256k1_v0_4_1_context* ctx = (rustsecp256k1_v0_4_1_context*)checked_malloc(&default_error_callback, prealloc_size);
    if (EXPECT(rustsecp256k1_v0_4_1_context_preallocated_create(ctx, flags) == NULL, 0)) {
        free(ctx);
        return NULL;
    }
...
rustsecp256k1_v0_4_1_context* rustsecp256k1_v0_4_1_context_clone(const rustsecp256k1_v0_4_1_context* ctx) {
    rustsecp256k1_v0_4_1_context* ret;
    size_t prealloc_size;

    VERIFY_CHECK(ctx != NULL);
    prealloc_size = rustsecp256k1_v0_4_1_context_preallocated_clone_size(ctx);
    ret = (rustsecp256k1_v0_4_1_context*)checked_malloc(&ctx->error_callback, prealloc_size);
    ret = rustsecp256k1_v0_4_1_context_preallocated_clone(ctx, ret);
    return ret;
}
...
void rustsecp256k1_v0_4_1_context_destroy(rustsecp256k1_v0_4_1_context* ctx) {
    if (ctx != NULL) {
        rustsecp256k1_v0_4_1_context_preallocated_destroy(ctx);
        free(ctx);
    }
}
...
rustsecp256k1_v0_4_1_scratch_space* rustsecp256k1_v0_4_1_scratch_space_create(const rustsecp256k1_v0_4_1_context* ctx, size_t max_size) {
    VERIFY_CHECK(ctx != NULL);
    return rustsecp256k1_v0_4_1_scratch_create(&ctx->error_callback, max_size);
}

void rustsecp256k1_v0_4_1_scratch_space_destroy(const rustsecp256k1_v0_4_1_context *ctx, rustsecp256k1_v0_4_1_scratch_space* scratch) {
    VERIFY_CHECK(ctx != NULL);
...
int rustsecp256k1_v0_4_1_ecdsa_verify(const rustsecp256k1_v0_4_1_context* ctx, const rustsecp256k1_v0_4_1_ecdsa_signature *sig, const unsigned char *msg32, const rustsecp256k1_v0_4_1_pubkey *pubkey) {
    rustsecp256k1_v0_4_1_ge q;
    rustsecp256k1_v0_4_1_scalar r, s;
    ...
    rustsecp256k1_v0_4_1_scalar_set_b32(&m, msg32, NULL);
    rustsecp256k1_v0_4_1_ecdsa_signature_load(ctx, &r, &s, sig);
...
int rustsecp256k1_v0_4_1_ecdsa_sign(const rustsecp256k1_v0_4_1_context* ctx, rustsecp256k1_v0_4_1_ecdsa_signature *signature, const unsigned char *msg32, const unsigned char *seckey, rustsecp256k1_v0_4_1_nonce_function noncefp, const void* noncedata) {
    rustsecp256k1_v0_4_1_scalar r, s;
    int ret;
    VERIFY_CHECK(ctx != NULL);
    ...
    ret = rustsecp256k1_v0_4_1_ecdsa_sign_inner(ctx, &r, &s, NULL, msg32, seckey, noncefp, noncedata);
...
static int rustsecp256k1_v0_4_1_ec_seckey_tweak_add_helper(rustsecp256k1_v0_4_1_scalar *sec, const unsigned char *tweak) {
    rustsecp256k1_v0_4_1_scalar term;
    int overflow = 0;
    int ret = 0;

    rustsecp256k1_v0_4_1_scalar_set_b32(&term, tweak, &overflow);
    ret = (!overflow) & rustsecp256k1_v0_4_1_eckey_privkey_tweak_add(sec, &term);
    rustsecp256k1_v0_4_1_scalar_clear(&term);
    return ret;
}

int rustsecp256k1_v0_4_1_ec_seckey_tweak_add(const rustsecp256k1_v0_4_1_context* ctx, unsigned char *seckey, const unsigned char *tweak) {
    rustsecp256k1_v0_4_1_scalar sec;
    int ret = 0;
    VERIFY_CHECK(ctx != NULL);
    ARG_CHECK(seckey != NULL);
    ...
    ret = rustsecp256k1_v0_4_1_scalar_set_b32_seckey(&sec, seckey);
    ret &= rustsecp256k1_v0_4_1_ec_seckey_tweak_add_helper(&sec, tweak);
    rustsecp256k1_v0_4_1_scalar_cmov(&sec, &rustsecp256k1_v0_4_1_scalar_zero, !ret);
    rustsecp256k1_v0_4_1_scalar_get_b32(seckey, &sec);
...
int rustsecp256k1_v0_4_1_ec_privkey_tweak_add(const rustsecp256k1_v0_4_1_context* ctx, unsigned char *seckey, const unsigned char *tweak) {
    return rustsecp256k1_v0_4_1_ec_seckey_tweak_add(ctx, seckey, tweak);
}

static int rustsecp256k1_v0_4_1_ec_pubkey_tweak_add_helper(const rustsecp256k1_v0_4_1_ecmult_context* ecmult_ctx, rustsecp256k1_v0_4_1_ge *p, const unsigned char *tweak) {
    rustsecp256k1_v0_4_1_scalar term;
    int overflow = 0;
    rustsecp256k1_v0_4_1_scalar_set_b32(&term, tweak, &overflow);
...
int rustsecp256k1_v0_4_1_ec_pubkey_tweak_add(const rustsecp256k1_v0_4_1_context* ctx, rustsecp256k1_v0_4_1_pubkey *pubkey, const unsigned char *tweak) {
    rustsecp256k1_v0_4_1_ge p;
    int ret = 0;
    VERIFY_CHECK(ctx != NULL);
    ARG_CHECK(rustsecp256k1_v0_4_1_ecmult_context_is_built(&ctx->ecmult_ctx));
    ...
    ret = rustsecp256k1_v0_4_1_pubkey_load(ctx, &p, pubkey);
    memset(pubkey, 0, sizeof(*pubkey));
    ret = ret && rustsecp256k1_v0_4_1_ec_pubkey_tweak_add_helper(&ctx->ecmult_ctx, &p, tweak);
    if (ret) {
        rustsecp256k1_v0_4_1_pubkey_save(pubkey, &p);
    }
...
int rustsecp256k1_v0_4_1_ec_seckey_tweak_mul(const rustsecp256k1_v0_4_1_context* ctx, unsigned char *seckey, const unsigned char *tweak) {
    ...
    rustsecp256k1_v0_4_1_scalar_set_b32(&factor, tweak, &overflow);
    ret = rustsecp256k1_v0_4_1_scalar_set_b32_seckey(&sec, seckey);
...
int rustsecp256k1_v0_4_1_ec_privkey_tweak_mul(const rustsecp256k1_v0_4_1_context* ctx, unsigned char *seckey, const unsigned char *tweak) {
    return rustsecp256k1_v0_4_1_ec_seckey_tweak_mul(ctx, seckey, tweak);
}

int rustsecp256k1_v0_4_1_ec_pubkey_tweak_mul(const rustsecp256k1_v0_4_1_context* ctx, rustsecp256k1_v0_4_1_pubkey *pubkey, const unsigned char *tweak) {
    rustsecp256k1_v0_4_1_ge p;
    rustsecp256k1_v0_4_1_scalar factor;
    int ret = 0;
    int overflow = 0;
