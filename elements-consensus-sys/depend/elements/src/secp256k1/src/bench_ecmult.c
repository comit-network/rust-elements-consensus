...
static int bench_callback(rustsecp256k1_v0_4_1_scalar* sc, rustsecp256k1_v0_4_1_ge* ge, size_t idx, void* arg) {
...
int main(int argc, char **argv) {
    ...
    pubkeys_gej = malloc(sizeof(rustsecp256k1_v0_4_1_gej) * POINTS);
    rustsecp256k1_v0_4_1_gej_set_ge(&pubkeys_gej[0], &rustsecp256k1_v0_4_1_ge_const_g);
    rustsecp256k1_v0_4_1_scalar_set_int(&data.seckeys[0], 1);
    for (i = 0; i < POINTS; ++i) {
        generate_scalar(i, &data.scalars[i]);
        if (i) {
            rustsecp256k1_v0_4_1_gej_double_var(&pubkeys_gej[i], &pubkeys_gej[i - 1], NULL);
            rustsecp256k1_v0_4_1_scalar_add(&data.seckeys[i], &data.seckeys[i - 1], &data.seckeys[i - 1]);
        }
    }
    rustsecp256k1_v0_4_1_ge_set_all_gej_var(data.pubkeys, pubkeys_gej, POINTS);
    free(pubkeys_gej);
